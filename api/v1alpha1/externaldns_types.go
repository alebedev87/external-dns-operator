...
type ExternalDNSGCPProviderOptions struct {
	...
	// and https://github.com/kubernetes-sigs/external-dns/blob/master/docs/tutorials/gke.md
	// for more information
	//
	// NOTE
	//
	// This field is not supported by ExternalDNS.
	// In order to be able to access GCP DNS, ExternalDNS' service account has to be given with GCP DNS admin IAM role.
	// There are multiple ways of doing this, see the above mentioned tutorial.
	//
	// +kubebuilder:validation:Required
	// +required
	Credentials NamespacedSecretReference `json:"credentials"`
...
type ExternalDNSSource struct {
	...
	//
	// Especially useful when HostnameAnnotationPolicy is set to Ignore.
	//
