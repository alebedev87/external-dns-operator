...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if _, _, err := r.ensureExternalDNSClusterRole(ctx); err != nil {
		return reconcile.Result{}, fmt.Errorf("failed to ensure externalDNS cluster role: %v", err)
	}
	...
	if _, _, err := r.ensureExternalDNSClusterRoleBinding(ctx, r.config.Namespace, externalDNS); err != nil {
		return reconcile.Result{}, fmt.Errorf("failed to ensure externalDNS cluster role binding: %v", err)
	}
